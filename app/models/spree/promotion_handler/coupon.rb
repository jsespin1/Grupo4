      ...
      def apply
        if order.coupon_code.present?
          if promotion.present? && promotion.actions.exists?
            puts "PROMOCION: " << promotion.inspect
            product = @order.line_items[0]
            sku_order = product.sku.to_i
            sku_coupon = promotion.description.split(":").last.to_i
            puts "Sku coupon: " << sku_coupon.to_s
            if sku_order != sku_coupon
